...
const bind_setter = (obj, k, v, proxy)=>{
...
const bind = new Proxy({}, { set: bind_setter });

const react_getter = (obj, key, proxy)=>{
	window[key] = window[key];
	return proxy;
}; const $react = new Proxy({}, { get: react_getter });

const defined_getter = (obj, key, proxy)=>{
	return window[key] != undefined;
}; const $defined = new Proxy({}, { get: defined_getter });
...
// 		const to_join = split.map( (s, i) => {
// 			const part = args.at(i-1);
// 			if (s.startsWith("}")) s = s.replace("}", part);
// 			else if (s.startsWith("?}")) s = s.replace("?}", JSON.stringify(part));

// 			return s;
// 		});
// 		console.log(to_join.join(""));
// 	}
// 	printf("My name is {}, and my age is {}", "Jacoby", 18);
// 	const obj = { str: "String", num: 42 };
// 	printf("Print and object: {}\nNo wait... this!: {?}", obj, obj);
// }